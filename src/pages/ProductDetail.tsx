...
import { fetchProductById as supabaseFetchProduct, fetchAllProducts } from "@/lib/productService";
...
const ProductDetail = () => {
  ...
  const { data: supabaseProduct } = useQuery({
    queryKey: ['product', id],
    queryFn: () => supabaseFetchProduct(id!),
    enabled: !!id,
    retry: 1,
  });
  ...
  const product = supabaseProduct || (id ? getProductById(id) : null);
  const relatedProducts = product
    ? getRelatedProducts(product.id, product.category, 4, allProducts)
    : [];
